...
export default async function handler(req, res) {
  try {
    ...
    const query = 'SELECT * FROM informatiocontributors WHERE email=$1 AND password=$2';
    ...
    if (data.rows.length > 0) {
      const memberinfo = {
        ...
        gender: data.rows[0].gender,
          ...
          // Create a JWT token
          const jwttoken2 = jwt.sign(memberinfo, process.env.MEMBERSIGNINCOOKIE);

          // Set the JWT token as a cookie using the 'cookie' package
          res.setHeader('Set-Cookie', cookie.serialize('membercookie', jwttoken2, {
            httpOnly: true,
