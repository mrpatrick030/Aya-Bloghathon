...
export default async function handler(req, res) {
  try {
    ...
    // Define the SQL query with placeholder for id
    const query = 'SELECT * FROM informatioblog WHERE id=$1';
    const values = [id];
    ...
    if (data.rows.length > 0) {
      const blogpostinfo = {
        // blog data
        image_link: data.rows[0].image_link,
        video_link: data.rows[0].video_link,
        title: data.rows[0].title,
        date: data.rows[0].date,
        contributor: data.rows[0].contributor,
        description: data.rows[0].description,
        category: data.rows[0].category,
      };

          // Create a JWT token
          const jwttoken3 = jwt.sign(blogpostinfo, process.env.BLOGCOOKIE);

          // Set the JWT token as a cookie using the 'cookie' package
          res.setHeader('Set-Cookie', cookie.serialize('blogpostcookie', jwttoken3, {
            httpOnly: true,
